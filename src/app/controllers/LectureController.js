...
class LectureController {
  ...
  lecture(req, res, next) {
    Lecture.findById(req.session.lectureId)
      .populate('listCourse')
      .then((lect) => {
  ...
  showInfo(req, res, next) {
    ...
    Lecture.findById(req.session.lectureId).then((lect) =>
  ...
  async changePassword(req, res, next) {
    ...
    if (lecture.password !== req.body.password) {
      ...
      await Lecture.findByIdAndUpdate(req.body.id, {
  ...
  async homeWork(req, res, next) {
    const lect = await Lecture.findById(req.session.lectureId).populate(
      'listCourse',
      '_id'
    );
    ...
    for (var i = 0; i < lect.listCourse.length; i++) {
      listHomeworks.push(
        await SubmitHomework.find({
          idCourse: lect.listCourse[i]._id,
        }).populate('idCourse idStudent', 'name')
      );
    }
    // console.log(arr)
    // return res.json(listHomeworks)

    res.render(path.join('lecture', 'lecture-homework'), {
      lect,
      listHomeworks,
    });
  }
