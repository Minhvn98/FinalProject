...
class StudentController {
  ...
  async student(req, res, next) {
    const student = await Student.findById(req.session.studentId).populate(
      'listCourses.idCourse'
    );
    const requireCourses = await Course.find({}, null, {
      sort: { createdAt: -1 },
      limit: 4,
    });
    //console.log(req)
  ...
  showInfo(req, res, next) {
    Student.findById(req.session.studentId)
      .then((student) =>
        res.render(path.join('student', 'student-info'), { student })
      )
      .catch((err) => next(err));
  }
  ...
  async homeWork(req, res, next) {
    ...
    const processHomework = SubmitHomework.find({
    ...
    const cc = await Course.findById('5faf33138f0a2e23f86b5be8').populate(
      'lessons'
    );
    // return res.json(homeworks)
    res.render(path.join('student', 'student-homework'), {
      student,
      homeworks,
      cc,
    });
  ...
  getNotification(req, res, next) {
    Notification.find({ idUserReceived: req.session.studentId }, null, {
      sort: { createdAt: -1 },
    })
